...
object print {
  ...
  //     case mu.InjMap(TMap(value))                   => s"Map[String, $value]"
  //     case mu.InjGeneric(TGeneric(generic, params)) => s"""$generic[${params.mkString(", ")}]"""
  //     case mu.InjList(TList(value))                 => s"List[$value]"
  //     case mu.InjRequired(TRequired(value))         => value
  //     case mu.InjCoproduct(TCoproduct(invariants)) =>
  //       invariants.toList.mkString("Cop[", " :: ", ":: TNil]")
  //     case mu.InjSum(TSum(name, fields)) =>
